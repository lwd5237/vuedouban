...
export function createMovieList(movieList) {
  ...
  movieList.forEach((movie, index) => {
    ret.push(new Movie({
      ...
      image: attachImageUrl(movie.images.large),
...
export function createMovieList1(movie) {
  let ret = []
    ret.push(new Movie({
      id: movie.id,
      title: movie.title,
...
export function createRankList(rankList) {
  ...
  rankList.forEach((item) => {
    ...
    ret.push(new Movie({
      ...
      image: attachImageUrl(item.images.large)
    }))
  })
  return ret
};
...
export function createSearchList(movieList) {
  ...
  movieList.forEach((movie, index) => {
    ret.push(new Movie({
      ...
      rating: movie.rating.average,
      image: attachImageUrl(movie.images.large),
      pubdates: filterTime(movie.pubdates),
...
//预处理时间信息
function filterTime(pubdates) {
  let time = ''
  pubdates.forEach((item, index) => {
    if (item.indexOf('(中国大陆)')) {
      time = item.slice(0, item.indexOf('('))
    }
  })
  return time
}
